export const ru = () =>{
    ...
    const keys2 = ['Tab', 'Й', 'Ц', 'У', 'К', 'Е', 'Н', 'Г', 'Ш', 'Щ', 'З', 'Х', 'Ъ', '\\'];
    keys2.forEach(key => {
    ...
    const keys3 = ['Caps Lock', 'Ф', 'Ы', 'В', 'А', 'П', 'Р', 'О', 'Л', 'Д', 'Ж', "Э", 'Enter'];
    keys3.forEach(key => {
      const keyElement = document.createElement('div');
      keyElement.classList.add('keys');
      keyElement.textContent = key;
    ...
    const keys4 = ['Shift', 'Я', 'Ч', 'С', 'М', 'И', 'Т', 'Ь', 'Б', 'Ю', '/', '?', 'Shift'];
    keys4.forEach(key => {
      const keyElement = document.createElement('div');
      keyElement.classList.add('keys');
      keyElement.textContent = key;
    ...
    const keys5 = ['Ctrl', 'Win', 'Alt', 'Space', 'Alt', 'Fn', 'Ctrl']; keys5.forEach(key => {
      const keyElement = document.createElement('div');
      keyElement.classList.add('keys');
      keyElement.textContent = key;
